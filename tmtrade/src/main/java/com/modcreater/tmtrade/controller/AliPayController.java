...
public class AliPayController {
    ...
    public static String NOTIFY_URL = "goku-yi.gnway.cc/alipay/notify_url.do";
    public static String RETURN_URL = "goku-yi.gnway.cc/alipay/return_url.do";
    ...
    public Dto aliPayOrderSubmitted(@RequestBody ReceivedOrderInfo receivedOrderInfo, HttpServletRequest httpServletRequest) throws Exception{
        ...
        request.setNotifyUrl("goku-yi.gnway.cc/alipay/notify_url.do");
        ...
        try {
            ...
            // 签约合作者身份ID
            String orderInfo = "partner=" + "\"" + APP_ID + "\"";

            // 签约卖家支付宝账号
            orderInfo += "&seller_id=" + "\"" + SELLER_ID + "\"";

            // 商户网站唯一订单号
            orderInfo += "&out_trade_no=" + "\"" + userOrder.getId() + "\"";
            // getOutTradeNo()
            // 商品名称
            orderInfo += "&subject=" + "\"" + "手机端"+userOrder.getOrderTitle()+"移动支付" + "\"";

            // 商品详情
            orderInfo += "&body=" + "\"" + "您花费"+userOrder.getPaymentAmount()+"元" + "\"";

            // 商品金额
            orderInfo += "&total_fee=" + "\"" + userOrder.getPaymentAmount() + "\"";
