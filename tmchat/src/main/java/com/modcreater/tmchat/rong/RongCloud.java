 ...
 * v2.0.1
 */
package com.modcreater.tmchat.rong;
...
public class RongCloud {
	...
	private static ConcurrentHashMap<String, io.rong.RongCloud> rongCloud = new ConcurrentHashMap<String, io.rong.RongCloud>();
	...
	public static void setApiHostListBackUp(List<HostType> apiHostListBackUp) {
		io.rong.RongCloud.apiHostListBackUp = apiHostListBackUp;
	}
	...
	public static io.rong.RongCloud getInstance(String appKey, String appSecret) {
		if (null == rongCloud.get(appKey)) {
			rongCloud.putIfAbsent(appKey, new io.rong.RongCloud(appKey, appSecret));
	 ...
	 * 自定义 API 地址
	 * */
	public static io.rong.RongCloud getInstance(String appKey, String appSecret, String api) {
		if (null == rongCloud.get(appKey)) {
			io.rong.RongCloud rc =  new io.rong.RongCloud(appKey, appSecret);
	 ...
	 * */
	public static io.rong.RongCloud getInstance(String appKey, String appSecret, String api, List<String> apiBackUp) {
		if (null == rongCloud.get(appKey)) {
			io.rong.RongCloud rc =  new io.rong.RongCloud(appKey, appSecret);
			if(api!=null && api.trim().length()>0){
				rc.setApiHostType(new HostType(api));
